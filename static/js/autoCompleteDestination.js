(function (global, factory) {
    ...
    (global.autoCompleteDestination = factory());
    ...
    var getSearchInput = function getSearchInput() {
      return document.querySelector("#autoCompleteDestination");
    };
    ...
    var addResultsToList = function addResultsToList(results, cleanResults, dataAttribute) {
      var resultState;
      var resultsList = document.querySelectorAll(".autoComplete_results_list")[1];
      results.forEach(function (event, record) {  
        var result = document.createElement("li");
  
        // Dot
        // elemento y añadimos la clase correspondiente con cleanResults[record].color
        const dot = document.createElement('div');
        dot.className = `dot_autoComplete ${cleanResults[record].color}`
        result.appendChild(dot)
  
        // Texto
        result.setAttribute("data-".concat(dataAttribute.tag), dataAttribute.value || cleanResults[record].name);
        ...
        result.setAttribute("id", cleanResults[record].name);
        result.setAttribute("class", resultState);
        
        const span = document.createElement('span')
        span.innerHTML = results[record];
        ...
        value: function listMatchedResults() {
          ...
          try {
            this.dataSrc().filter(function (record) {
              var match = _this.search(inputValue, record.name);
              if (match) {
